...
- **[docs/adr/](docs/adr/)** — irreversible trade-offs. Read the matching ADR before changing isolation, the Cache, the Environment, the Daemon API, or the Host OS/hypervisor.

## How to work here

- Enter the env with `devenv shell -- <cmd>`. Do not use host Python/Node/toolchains.
- Host runners and scripts live **inline in Nix** (`writeShellApplication` in `nix/*.nix`). Do not add standalone `.sh` files.
