...
- The bundled UI is a Solid 2 + Vite + Tailwind SPA built with Bun (`canvas/`) and served by the Daemon, a client of the documented API ([0021](docs/adr/0021-ui-is-a-solid-spa.md)). The GUI is a Canvas of Windows ([0020](docs/adr/0020-gui-is-a-canvas-of-windows.md)). Build with `devenv shell -- bun --cwd canvas run build` so `canvas/dist` exists; the Daemon serves that directory.
