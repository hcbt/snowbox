...
- Environment flakes live on the Host (`sandboxes/{id}/environment`, default from `environment/empty`). Their nixpkgs input is `nixos-unstable` so Packages are current; the guest OS pin is separate. Workspace is not a virtiofs. Cache is `data/snowbox/cache`.
