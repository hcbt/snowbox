...
The Environment is a Host document owned by Snowbox. The GUI and Template hatch write it; the Sandbox only receives realized Packages. A flake in `/workspace` is the project’s. The Agent cannot persist a Package add.
