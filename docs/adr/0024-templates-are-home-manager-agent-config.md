...
A Template is still a Nix flake ([0005](0005-templates-are-nix-flakes.md)). What it means is devenv plus a home-manager configuration of first-class Agents (`programs.claude-code`, `programs.pi-coding-agent`, `programs.codex`, …). The hatch is a Canvas form over those home-manager options, not a Snowbox schema. Save writes the flake; New Sandbox realizes it.
...
Secrets do not belong in the flake: home-manager `apiKey` options would land in the Cache. Tokens go through Place a Secret into Home ([0004](0004-home-is-an-allowlist.md)).
