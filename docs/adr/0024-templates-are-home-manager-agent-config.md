...
A Template is still a Nix flake ([0005](0005-templates-are-nix-flakes.md)). What it means is devenv plus a home-manager configuration of first-class Agents (`programs.claude-code`, `programs.pi-coding-agent`, `programs.codex`, …). The Environment form is a Canvas form over those home-manager options, not a Snowbox schema. Save writes the flake; New Sandbox realizes it.
